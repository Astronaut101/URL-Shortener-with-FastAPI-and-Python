...
def create_db_url(db: Session, url: schemas.URLBase) -> models.URL:
    key = keygen.create_random_keys(db)
    secret_key = f"{key}_{keygen.create_random_keys(length=8)}"
    secret_key = key.create_random_keys(length=8)
    db_url = models.URL(
...
def get_db_url_by_key(db: Session, secret_key: str) -> models.URL:
    return (
        db.query(models.URL)
        .filter(models.URL.key == secret_key, models.URL.is_active)
        .first()
    )
